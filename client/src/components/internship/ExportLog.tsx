...
import { exportInternshipLog } from "../../features/internship/internshipActions";
import { useParams } from "react-router";

interface ExportLogProps {
  isExportModalOpen: boolean; // The state value
  setIsExportModalOpen: Dispatch<SetStateAction<boolean>>; // The state setter function
}
...
const ExportLog = ({
  ...
  const handleExportLog = (e: SyntheticEvent) => {
    e.preventDefault();
    dispatch(exportInternshipLog({ internshipId: numberId })).unwrap();
  };
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-zinc-900/40 backdrop-blur-sm p-4">
      <div className="w-full max-w-lg rounded-xl bg-white shadow-2xl ring-1 ring-zinc-200 overflow-hidden flex flex-col">
        ...
        <div className="border-b border-zinc-100 px-6 py-5 flex items-center justify-between bg-zinc-50/50">
          <div>
            <h2 className="text-lg font-semibold text-zinc-900">Export Logs</h2>
